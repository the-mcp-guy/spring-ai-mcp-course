...
> [!NOTE]
> **This branch is Class 8.** It implements
> [Class 8: Consuming Resources and Prompts](https://themcpguy.com/docs/mcp-spring-ai/consuming-resources-and-prompts/),
> which puts the server's resources and prompts to work on the client side: the agent
> reads the returns policy into the system prompt on every chat, attaches the selected
> order through the `order://{orderId}` template, and serves the `draft_refund_email`
> prompt behind a new `/api/refund-email` endpoint. Classes 2 to 5 built the server side:
...
> client, and [Class 7](https://themcpguy.com/docs/mcp-spring-ai/tools-to-a-model) gave
> it a model. `main` stays at the course starting point, with no AI or MCP code at all,
> so clone that branch to follow along from Class 1.
...
support-agent/          the agent: an MCP client since Class 6, with a model since Class 7,
                        reading resources and running prompts since Class 8.
...
On startup it still prints what the server offers. The listing gained a line in this
class:
...
`support-agent/src/main/resources/application.yaml`. The `template` line is new: templates
are not returned by `resources/list`, so the inspector asks for them separately with
`resources/templates/list` since this class.
...
this class that surface has grown:
...
cancelled order calls `/api/refund-email`; both are answered by the agent from this class
