...
> [!NOTE]
> **This branch is the completed course.** It holds the code as the classes leave
> it, finishing with
> [Class 16: Guardrails and Budgets](https://themcpguy.com/docs/mcp-spring-ai/guardrails-and-budgets/),
> which wraps the model call in two advisors: a blocklist that refuses a question
> containing a blocked word, and a token budget that stops a conversation once it
> has spent its allowance. On top of that sits the stdio profile from
> [Class 17: Into Claude Desktop](https://themcpguy.com/docs/mcp-spring-ai/into-claude-desktop/),
> which lets Claude Desktop start the order service as a local MCP server; that
> class does not get a branch of its own, so its change lives only here.
> [Class 15: Async and Stateless](https://themcpguy.com/docs/mcp-spring-ai/async-and-stateless/)
> flips the server to ASYNC and to STATELESS to see what each one changes, then
> puts both back, so it leaves the code exactly as Class 14 left it.
> [Class 14: Testing](https://themcpguy.com/docs/mcp-spring-ai/testing/)
> covers both applications with tests that never call a model. `order-service`
...
support-agent/          the agent: an MCP client since Class 6, with a model since Class 7,
                        ...
                        roots and sending _meta since Class 13, carrying its own
                        tests since Class 14, and guarding the model call with a
                        blocklist and a token budget since Class 16.
